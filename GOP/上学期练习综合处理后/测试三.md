...
- [x] **1.** Bedingte Wahrscheinlichkeiten definieren, für eine feste Bedingung, eine neue Wahrscheinlichkeitsverteilung auf der Teilmenge der Grundgesamtheit, die der Bedingung entspricht.  
...
- [x] **4.** $P(A \mid B)$ bedeutet: „Die Wahrscheinlichkeit, dass, angenommen $B$ tritt ein, auch Ereignis $A$ eintritt.“  
...
- [x] **11.** Es gilt immer: $P(A \mid B) \geq P(A \cap B)$  

- [ ] **12.** Es gilt immer: $P(A \mid B) \leq P(A \cap B)$  

- [x] **13.** Der „Satz von der totalen Wahrscheinlichkeit“ besagt, dass man sich die unbedingte Wahrscheinlichkeit eines Ereignisses aus bedingten Wahrscheinlichkeiten für das Ereignis und den Wahrscheinlichkeiten dieser Bedingungen ausrechnen kann.  

- [x] **14.** Die Mengen der einzelnen Elementarereignisse $\{\omega_i\}$ bilden eine disjunkte Zerlegung der Grundgesamtheit $\Omega$.  
...
- [x] **3.** Sei $A$ ein beliebiges Ereignis. Es gilt immer $P(A \mid \Omega)=P(A)$.  

- [x] **4.** Falls das Ereignis $A$ eine Teilmenge von dem Ereignis $B$ beschreibt, so gilt $P(B \mid A)=1$.  

- [x] **5.** Wenn $B_1, B_2$ eine disjunkte Zerlegung von $\Omega$ darstellt, also $B_1 \dot{\cup} B_2=\Omega$, dann gilt $P(B_1 \mid B_2)=0$.  
...
- [x] **6.** $P(Z \mid X)=0.\overline{96}$  
  **$P(Z \mid X)=0.\overline{96}$**

- [x] **7.** $P(X \cap Z) \leq P(Z)$  
  **$P(X \cap Z) \leq P(Z)$**

- [x] **8.** $P(X \cap Z) \leq P(X)$  
  **$P(X \cap Z) \leq P(X)$**
...
- [x] **12.** $P(Z \cap \overline{X})$ ist die Wahrscheinlichkeit, dass Xaver seine Maß nicht über den Kopf schüttet, Zenzi dem Xaver trotzdem ihre über den Kopf schüttet.  

- [x] **13.** $P(Z \mid \overline{X})$ ist die Wahrscheinlichkeit, dass, wenn Xaver Zenzi seine Maß nicht über den Kopf schüttet, Zenzi dem Xaver trotzdem ihre über den Kopf schüttet.  

- [x] **14.** Die Wahrscheinlichkeit, dass, wenn Xaver Zenzi seine Maß nicht über den Kopf schüttet, Zenzi dem Xaver trotzdem ihre über den Kopf schüttet, ist ca. $0.124$.  
...
- [x] **1.** Nach dem Satz von Bayes kann eine bedingte Wahrscheinlichkeit $P(B \mid A)$ über die umgekehrte bedingte Wahrscheinlichkeit $P(A \mid B)$ und die Einzelwahrscheinlichkeiten $P(A)$, $P(B)$ formuliert werden.  
...
- [x] **3.** Bei diagnostischen Tests ist bezüglich der Aussagekraft des Testergebnisses nicht nur die Genauigkeit des Tests, sondern insbesondere auch der Anteil positiver bzw. negativer Fälle in der Grundgesamtheit von Bedeutung.  
...
- [x] **1.** Mit diesen Tests wird bei bis zu $30\%$ aller getesteten Infizierten keine Infektion entdeckt.  
...
- [x] **6.** Die Odds für ein positives Testergebnis bei einer tatsächlich COVID-19-negativen Person sind $1:19$.  
...
- [x] **9.** Die Likelihoodratio $\frac{P(T \mid K)}{P(T \mid \overline{K})}$ eines positiven Testergebnisses für das Vorliegen einer Infektion ist mindestens $14$.  

- [x] **10.** Die Likelihoodratio $\frac{P(\overline{T} \mid \overline{K})}{P(\overline{T} \mid K)}$ eines negativen Testergebnisses für die Abwesenheit einer Infektion ist $\approx 3$.  
