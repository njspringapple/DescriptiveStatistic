...
译：设 $X\sim F$。求 $\mathbb P(X=0)$、$\mathbb P(X=1)$ 以及 $\mathbb P\left(X\in\left[\frac13,\frac23\right]\right)$。
...
译：对 $\lambda>0$，函数 $F:\mathbb R\to\mathbb R$ 定义如下：
...
译：设 $X$ 是参数为 $p\in(0,1)$ 的几何分布随机变量，即 $X$ 的计数密度为：
...
译：设 $X\sim\operatorname{LogN}(\mu,\sigma^2)$，其密度为：
...
译：用变量变换定理证明 $Y:=\log(X)\sim N(\mu,\sigma^2)$。
...
译：某酒馆每周进一次啤酒。设每周啤酒消耗量 $X$（单位：百升）具有如下密度：
...
译：拜仁平均每场进 $2.8$ 球。若两队进球数相互独立，计算拜仁 $4:0$ 获胜的概率。

###### (d)

Welche Verteilung hat die Wartezeit auf das nächste HSV-Tor?

译：等待 HSV 下一粒进球的时间服从什么分布？
...
译：$10$ 把钥匙中有一把能打开。每次失败后钥匙重新混合。设 $X$ 为直到成功所需的尝试次数。
...
译：某慕尼黑人平均每 $1000$ 个居民认识一个。一次散步中他遇到 $50$ 个慕尼黑人。设 $X$ 为其中熟人的数量。
...
译：设 $X$ 为连续随机变量，密度为 $f$，分布函数为 $F$。判断下列说法对错。
...
译：求 $a\in\mathbb R$，使得 $G_a$ 是一个分布函数。

###### (b)

Sei nun $X\sim G_a$ mit $a=\frac23$. Bestimmen Sie:

译：现在设 $X\sim G_a$ 且 $a=\frac23$。求：
...
译：设 $X\sim\operatorname{Geo}(p)$，即 $X:\Omega\to\mathbb N$ 服从几何分布，并满足：
...
译：设 $X\sim N(-5,1)$、$Y\sim N(3,5)$、$Z\sim t(10)$。随机变量 $V$ 服从由这三个随机变量按权重 $0.2,0.3,0.5$ 组成的混合分布。
...
译：设 $X$ 是密度为 $f_X$ 的随机变量，$g:\mathbb R\to\mathbb R$ 可逆，且 $h=g^{-1}$ 连续可微。推导一维变量变换定理。
...
译：随机在街上捡到一片树叶，其长度 $X$（单位：分米）服从具有如下密度函数的分布：
...
译：求随机树叶面积 $Y$ 的密度。树叶面积可以取哪些值，也就是说下式在哪些值上成立：
...
译：给定一组服从 $\mathcal U[0,1]$ 的随机数样本。简要说明可用什么方法以及如何由此生成 Poisson 分布随机数。（2 分）
...
译：它们是 $n\in\mathbb N$ 时的随机变量。定义“$X_1,X_2,X_3$ 相互独立”。（1 分）
...
译：下图显示 Poisson 分布样本的标准化对数似然。请在图中画出参数 $\lambda$ 的 $0.95\%$ 似然区间。（3 分）

**Hinweis:** Die Verteilungsfunktion der $\chi_1^2$-Verteilung finden Sie im Anhang.

译：**提示：** $\chi_1^2$ 分布的分布函数可在附录中找到。
...
译：对检验问题 $H_0$ 对 $H_1$ 进行水平为 $\alpha$ 的检验 $\psi$。检验结论为拒绝原假设。问这种决策可能发生哪类错误？能否给出该错误发生概率的最大值？（2 分）
...
译：设 $X_1,\dots,X_n$ 是独立同分布的连续随机变量，其密度为：
...
译：其中 $x_i>0$，分布参数为 $\alpha>0$、$\beta>0$。

a) Stelle $L(\alpha)$ und $l(\alpha)$ für gegebene Stichproben $x_1,\dots,x_n$ auf.

译：a) 对给定样本 $x_1,\dots,x_n$，写出 $L(\alpha)$ 和 $l(\alpha)$。
...
译：b) 令 $\beta=1$，由相应对数似然求 $\alpha$ 的最大似然估计量。可以假设所得结果确实对应最大值。
...
译：求常数 $c$，使得 $f(x)$ 是密度，并检验 $f(x)$ 是否满足 (a)(i) 中的性质。（4 分）
...
译：设对 $n\in\mathbb N$，$X_1,\dots,X_n$ 与随机变量 $X$ 独立同分布。现在考虑：
...
译：设 $\tilde Y_n$ 是 $Y_n$ 的标准化随机变量。对 $n\to\infty$，你能对 $\tilde Y_n$ 的收敛作出什么结论？为什么？（3 分）
...
译：某人发现即使完全不学习，每次考试也有 $20\%$ 的概率通过。平均需要参加多少次考试才能通过？相应方差是多少？
...
译：汉莎航空经验上约有 $18\%$ 的旅客不会乘坐已预订航班。为尽量提高飞机利用率，A320 的 $150$ 个座位会超售。若售出 $170$ 个座位，用中心极限定理计算实际想登机人数超过 $150$ 的概率，结果保留 $3$ 位小数。
...
译：对哪些 $a$ 和 $b$，$F(x)$ 是某个连续随机变量的分布函数？
...
译：画出密度草图。该分布的众数是什么？不做计算但给出理由，说明其期望、中位数和偏度。
...
译：证明当 $a=1$ 时，$F(x)$ 是某个连续随机变量的分布函数。
...
译：给出 $a=1$ 和 $a=0.5$ 时的 $P(X=1)$。

###### (f)

$0.25$-Quantil für $a=0.5$.

译：求 $a=0.5$ 时的 $0.25$ 分位数。
...
译：设 $X\sim\operatorname{Exp}(\lambda)$，其中 $\lambda\in\mathbb R^+$。
...
译：设 $Y=a+bX$，其中 $a,b\in\mathbb R^+$。说明为什么 $Y$ 是随机变量。
