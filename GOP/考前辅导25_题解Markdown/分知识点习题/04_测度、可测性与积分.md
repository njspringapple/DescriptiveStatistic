...
译：构造一个例子，使得在 $(\Omega,\mathcal F)$ 上测度唯一性定理要求的性质 i) 和 ii) 都成立，但仍有 $\mu_1\neq\mu_2$。
...
译：给定概率空间 $(\Omega,\mathcal F,\mu)$，其中：
...
译：设 $\Omega=\mathbb N$ 且 $\mathcal F=\mathcal P(\mathbb N)$。下列哪些映射 $\mu:\mathcal F\to\mathbb R$ 使 $(\Omega,\mathcal F,\mu)$ 成为测度空间？同时检查这些测度是否有限。
...
译：设测度空间为 $(\mathbb R,\mathcal B)$，并给定可测函数：
...
译：其中 $N\in\mathbb N$ 固定。分别对 Lebesgue 测度 $\lambda$ 和计数测度 $\mu_Z$，计算 $n\in\mathbb N, n\leq N$ 时在 $[0,n]$ 上的积分。
...
译：给定测度空间 $(\mathbb R,\mathcal B)$ 以及可测函数：
...
译：分别对 Lebesgue 测度 $\lambda$ 和计数测度 $\mu_Z$ 计算：

1. $\int_{[0,n]} f\,d\lambda$ und $\int_{[0,n]} f\,d\mu_Z$ für $n\in\mathbb N$,
2. $\int g\,d\lambda$ und $\int g\,d\mu_Z$.

译：1. 对 $n\in\mathbb N$，计算 $\int_{[0,n]} f\,d\lambda$ 和 $\int_{[0,n]} f\,d\mu_Z$；2. 计算 $\int g\,d\lambda$ 和 $\int g\,d\mu_Z$。
...
译：设 $y_3=a$ 且 $y_{k-1}<b<y_k$。推导 $\int_{[a,b]}f\,d\mu$。

###### (c)

$\lambda$ sei das Lebesguemaß. Leiten Sie $\int_{[a,b]}f\,d\lambda$ her.

译：设 $\lambda$ 为 Lebesgue 测度。推导 $\int_{[a,b]}f\,d\lambda$。
...
译：设 $\Omega=\mathbb N=\{1,2,\dots\}$ 且 $\mathcal F=\mathcal P(\mathbb N)$。下列哪些映射 $\mu:\mathcal F\to\mathbb R_0^+$ 使 $(\Omega,\mathcal F,\mu)$ 成为测度空间？并检查所有合法测度是否有限。
...
译：设 Lebesgue 测度 $\lambda:\mathcal B(\mathbb R)\to\mathbb R_0^+\cup\{\infty\}$ 满足：
...
译：设 $A\subset\mathbb R$ 可数。$\lambda(A)$ 等于什么？
...
译：由 $\mathcal E$ 生成的 $\sigma$-代数具有如下原子：
...
译：计算集合上的 Lebesgue-Stieltjes 测度 $\lambda_G$：
...
译：设 $\lambda_F$ 为与 $F$ 对应的 Lebesgue-Stieltjes 测度。

###### (1)

Bestimmen Sie $\lambda_F((-\infty,a])$ für beliebige $a\in\mathbb R$.

译：对任意 $a\in\mathbb R$，求 $\lambda_F((-\infty,a])$。
...
译：给定函数列 $f_n:\mathbb R\to\mathbb R$，$n\in\mathbb N$，其中：
...
译：证明函数 $f:\mathbb R\to\mathbb R$，其中：
...
译：不是 Riemann 可积，但 Lebesgue 可积，并计算：
...
译：对 $x\in\mathbb R$，在 $\mathbb R$ 上定义 Dirac 测度 $\delta_x$ 如下：
...
译：证明：$\delta_x$ 关于 Lebesgue 测度没有密度。
...
译：证明 $\mu$ 是测度空间 $(\Omega,\mathcal F)$ 上的一个测度。
...
译：在 $\Omega$ 上生成 $\sigma(\mathcal E)$。求 $\sigma(\mathcal E)$。
...
译：给出由 $\Omega$ 生成的 $\sigma$-代数上计数测度 $\mu_Z$ 的零测集。
...
译：证明：两个 $\sigma$-代数的并不一定是 $\sigma$-代数。为此在 $\Omega$ 上定义一个 $\sigma$-代数 $\mathcal F_2$，使得 $\mathcal F_1\cup\mathcal F_2$ 不是 $\sigma$-代数。

##### (c)

Zeigen Sie, dass $\mu$ ein Maß zum Messraum $(\Omega,\mathcal F_1)$ ist.

译：证明 $\mu$ 是测度空间 $(\Omega,\mathcal F_1)$ 上的测度。
...
译：$f$ 是否是某个关于支配测度 $\mu$ 的分布密度？请说明理由。
