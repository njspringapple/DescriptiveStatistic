...
译：计算 $\mathbb E(Y)$ 以及 $X\mid Y$ 的密度。
...
译：若 $X$ 与 $Y$ 独立，求 $\operatorname{Var}(W)$、$\operatorname{Var}(T)$、$\operatorname{Cov}(W,T)$ 和 $\rho(W,T)$。

###### (b)

Bestimmen Sie dieselben Größen, wenn $\rho(X,Y)=-\frac14$ gilt.

译：若 $\rho(X,Y)=-\frac14$，求同样这些量。

###### (c)

Warum gilt in Szenario (b) $\operatorname{Var}(W)<\operatorname{Var}(T)$?

译：为什么在情形 (b) 中有 $\operatorname{Var}(W)<\operatorname{Var}(T)$？
...
译：设 $X\sim U(0,1)$ 与 $Y\sim U(0,1)$ 独立。求下列随机变量的分布：
...
译：设 $X\sim\operatorname{Exp}(\lambda)$ 与 $Y\sim\operatorname{Exp}(\lambda)$ 独立。求 $Z=X+Y$ 的分布。
...
译：设 $X\sim\operatorname{Poi}(\lambda_X)$ 与 $Y\sim\operatorname{Poi}(\lambda_Y)$ 独立。求 $Z=X+Y$ 的分布。
...
译：设 $Z\sim\operatorname{Bernoulli}(0.5)$。在条件 $Z=0$ 下，设：
...
译：求 $\mathbb E(X^2)$ 和 $\operatorname{Var}(X)$。

###### (c)

Bestimmen Sie $\operatorname{Var}(X)$ mit dem Satz der totalen Varianz.

译：用总方差公式求 $\operatorname{Var}(X)$。
...
译：设 $X,Y$ 独立且都服从参数为 $\lambda$ 的 Poisson 分布。求 $Z=X+Y$ 的概率函数。

###### (b)

$A\sim\operatorname{Geom}(p)$ und $B\sim\operatorname{Geom}(p)$ seien unabhängig. Zeigen Sie:

译：设 $A\sim\operatorname{Geom}(p)$ 与 $B\sim\operatorname{Geom}(p)$ 独立。证明：
...
译：证明：对具有有限方差的随机变量 $X,Y$，相关系数不可能大于 $1$：
...
译：设 $X$ 和 $Y$ 随机独立且服从标准正态分布。用多元密度变换定理证明：
...
译：设 $X,Y\in\mathbb R$ 是独立随机变量且一阶矩有界。证明：
...
译：设 $Y_1,Y_2,X$ 为二阶矩有界的随机变量。证明协方差分解公式：
...
译：设 $X,Y\in\mathbb R$ 是二阶矩有界的随机变量。考虑线性模型：
...
译：其中 $\beta\in\mathbb R$，并且 $\varepsilon$ 与 $X$ 独立，且：
...
译：统计中常关心已收集数据的所谓经验密度函数。下面例子中收集了 $200$ 条数据，并观察每条数据的两个特征 $X$ 和 $Y$。下表给出观察结果：
...
译：b) 计算 $\operatorname{Cov}(X,Y)$ 和 $\varphi$。
...
译：推导 $X_1$ 和 $X_2$ 的边际密度。它们是否属于已知分布？
...
译：给出 $A$ 并说明理由。再给出 $B$ 尽可能大的下界并说明理由。
